...
class Title extends Text
{
    ...
    {
        /** @var \tyttam\PhpWord\Element\Title $element Type hint */
        $element = $this->element;
        $style = $element->getStyle();
        $style = str_replace('Heading', 'Heading_', $style);
        $style = \tyttam\PhpWord\Style::getStyle($style);
        if ($style instanceof \tyttam\PhpWord\Style\Font) {
            $this->fontStyle = $style;
            $pstyle = $style->getParagraph();
            if ($pstyle instanceof \tyttam\PhpWord\Style\Paragraph && $pstyle->hasPageBreakBefore()) {
                $sect = $element->getParent();
                if ($sect instanceof \tyttam\PhpWord\Element\Section) {
