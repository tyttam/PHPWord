...
class Word2007 extends AbstractWriter implements WriterInterface
{
    ...
    public function __construct(PhpWord $phpWord = null)
    {
        ...
        foreach (array_keys($this->parts) as $partName) {
            ...
            if (class_exists($partClass)) {
                /** @var \tyttam\PhpWord\Writer\Word2007\Part\AbstractPart $part Type hint */
                $part = new $partClass();
     ...
     *
     * @param \tyttam\PhpWord\Shared\ZipArchive $zip
        ...
        if (!empty($elements)) {
            foreach ($elements as $file => $media) {
                if (count($media) > 0) {
                    ...
                    /** @var \tyttam\PhpWord\Writer\Word2007\Part\AbstractPart $writerPart Type hint */
     ...
     *
     * @param \tyttam\PhpWord\Element\Section &$section
     * @param \tyttam\PhpWord\Shared\ZipArchive $zip
    ...
    private function addNotes(ZipArchive $zip, &$rId, $noteType = 'footnote')
    {
        ...
        /** @var \tyttam\PhpWord\Collection\AbstractCollection $collection Type hint */
    ...
    private function addChart(ZipArchive $zip, &$rId)
    {
        ...
        $index = 0;
        if ($collection->countItems() > 0) {
            /** @var \tyttam\PhpWord\Element\Chart $chart */
